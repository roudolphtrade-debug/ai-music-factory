...
const genres = ["Ambient Pop", "Hyperpop", "Techno", "Trap", "Neo-Soul", "Synthwave"];
const voices = ["Instrumental", "Warm tenor", "Soprano", "Alto", "Custom voice"];
  ...
  return (
    <div className="space-y-10">
      <SectionHeading
        ...
        description="Prompt, shape and master tracks with a workflow built for AI-native artists."
        action={
      ...
      <section className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        <div className="space-y-5 rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center gap-2">
            ...
            <span className="eyebrow text-gold">New composition</span>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            ...
            <Field label="Mood">
              <Select options={moods.slice(0, 6)} />
            </Field>
            <Field label="Voice">
          ...
          <div className="flex flex-wrap gap-3 border-t border-border pt-5">
            ...
            <Button variant="noir">Save draft</Button>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-[color-mix(in_oklab,var(--gold)_28%,transparent)] bg-card/40 p-8 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Drag a WAV or MP3 to import stems and master in-studio.
            </p>
            <Button variant="ghost-gold" size="sm" className="mt-4">
              Browse files
            </Button>
            <p className="mt-3 text-[0.7rem] text-muted-foreground/70">
              Audio engine connects later — UI ready
            </p>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <MiniStat icon={<Music2 className="h-4 w-4" />} label="Tracks" value="42" />
            <MiniStat icon={<Mic2 className="h-4 w-4" />} label="Voices" value="6" />
          </div>
        </div>
      </section>
      ...
      <section className="space-y-5">
        <SectionHeading eyebrow="Library" title="Your drafts & tracks" />
          ...
          {projects.map((p) => (
            ...
            >
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <GoldBadge variant="outline">{p.mood}</GoldBadge>
                <GoldBadge variant="outline">{p.voice}</GoldBadge>
              </div>

              <div className="mt-4 flex items-center gap-5 border-t border-border pt-4 text-xs text-muted-foreground">
                <span className="inline-flex items-center gap-1.5">
                  <GitBranch className="h-3.5 w-3.5 text-gold" /> {p.versions} versions
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <Layers className="h-3.5 w-3.5 text-gold" /> {p.stems} stems
                </span>
              </div>
            </article>
          ))}
